...
void PlacementEditor::synchronize() {
    ...
    for (unsigned i = 0; i < scene_->placement_transforms.size(); ++i)
        scene_->placement_transforms[i] = document_->transform(i);
...
void PlacementEditor::draw(const MaterialSelection &selection, const SpatialPoint *cursor) {
    poll();
    if (scene_ && selection.focus && selection.draw >= 0 &&
        scene_->draws.at(selection.draw).placement >= 0)
        ImGui::SetNextWindowFocus();
    ImGui::Begin("Object editing");
    if (!document_) {
        ImGui::TextWrapped("Load a map, then Ctrl+click a static object to edit its placement.");
        ImGui::End();
        return;
    }
    auto &io = ImGui::GetIO();
    ...
    if (project_store())
        ImGui::TextUnformatted(document_->dirty() ? "Unsaved project edits" : "No unsaved project edits");
    else
        ImGui::TextWrapped("%s", patch_path_.empty() ? "Patch not saved yet"
                                                   : patch_path_.filename().string().c_str());
    ...
    if (studio::TutorialWidgets::Button("placement_editor", "Redo"))
        document_->redo();
    ImGui::EndDisabled();
    if (!io.WantTextInput && !locked && io.KeyCtrl) {
    ...
    ImGui::Separator();
    if (selected_ < 0)
        ImGui::TextWrapped("Ctrl+click an object or choose it in Scene. Edit static placements here; "
                           "send terrain, props or non-player characters to Studio for model edits. "
                           "Use Spatial > Manage overworld to add or remove placements.");
    else {
        auto &e = document_->entries().at(selected_);
        ImGui::Text("Zone group %zu | Placement %zu", e.zone, e.row);
        ImGui::Text("Resource %u | Event %u", e.source.model, e.source.event);
        ...
        try {
            ...
            if (ImGui::DragFloat3("Position", state.position.data(), 1.f, -9999999, 9999999, "%.2f",
                                  ImGuiSliderFlags_AlwaysClamp))
                document_->preview(selected_, state);
            if (ImGui::IsItemDeactivatedAfterEdit())
                document_->commit();
            if (ImGui::DragFloat("Turn Y", &state.turn, .5f, -360000, 360000, "%.1f deg",
                                 ImGuiSliderFlags_AlwaysClamp))
                document_->preview(selected_, state);
            if (ImGui::IsItemDeactivatedAfterEdit())
                document_->commit();
            if (ImGui::IsItemHovered())
                ImGui::SetTooltip("Turn around the vertical axis from the original orientation; "
        ...
        studio::TutorialWidgets::RadioButton("placement_editor", "Move handles", &mode_, 0);
        ImGui::SameLine();
        studio::TutorialWidgets::RadioButton("placement_editor", "Turn ring", &mode_, 1);
        studio::TutorialWidgets::Checkbox("placement_editor", "Snap", &snap_);
        if (snap_) {
            ImGui::SetNextItemWidth(100);
            ImGui::DragFloat(mode_ ? "Degrees" : "Units", mode_ ? &angle_step_ : &step_, 1, 1, 1000,
                             "%.0f", ImGuiSliderFlags_AlwaysClamp);
        }
        ImGui::EndDisabled();
        if (!e.restriction.empty())
            ImGui::TextWrapped("%s", e.restriction.c_str());
        ImGui::TextWrapped(e.source.collision
    ...
    try {
        ...
        ImGui::SameLine();
        if (!project_store() && studio::TutorialWidgets::Button("placement_editor", "Save as..."))
            open_dialog(1);
        ...
        if (studio::TutorialWidgets::Button("placement_editor", "Export placement override...",
                                            {-1, 0})) {
            document_->compile();
            open_dialog(3);
        }
...
bool PlacementEditor::gizmo(const float *view, const float *projection, ImVec2 origin, ImVec2 size,
    ...
    if (mode_ == 0) {
        for (unsigned axis = 0; axis < 3; ++axis) {
            ...
            draw->AddLine(center, endpoints[axis], colors[axis], 3);
            draw->AddCircleFilled(endpoints[axis], 5, colors[axis]);
